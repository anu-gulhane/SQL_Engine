...
Page BufferManager::getPage(string matrixName, int rowIndex, int columnIndex)
{
    logger.log("BufferManager::getPage Matrix");
    string pageName = "../data/temp/"+matrixName + "_Page" + to_string(rowIndex)+"_"+to_string(columnIndex);
    logger.log(pageName);
    // if (this->inPool(pageName))
    //     return this->getFromPool(pageName);
    // else
        return this->insertIntoPool(matrixName, rowIndex, columnIndex);
}
...
Page BufferManager::insertIntoPool(string matrixName, int rowIndex, int colIndex)
{
    logger.log("BufferManager::insertIntoPool Matrix");
    Page page(matrixName,rowIndex,colIndex);
    logger.log("insert into pool matrix "+page.matrixName);
    if (this->pages.size() >= BLOCK_COUNT)
        pages.pop_front();
    pages.push_back(page);
    return page;
...
void BufferManager::writePage(string matrixName, int block_i,int block_j,vector<vector<int>> rows, int rowStart,int columnStart,int rowEnd,int columnEnd)
{
    logger.log("BufferManager::writePage-MATRIX");
    Page page(matrixName,block_i,block_j,rows,rowStart,columnStart,rowEnd,columnEnd);
    page.writePage(matrixName,rowStart,columnStart,rowEnd,columnEnd);
}
