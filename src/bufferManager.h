...
class BufferManager{
    ...
    Page insertIntoPool(string matrixName, int rowIndex, int colIndex);

    public:
    
    BufferManager();
    Page getPage(string tableName, int pageIndex);
    Page getPage(string matrixName, int rowIndex, int columnIndex);
    ...
    void writePage(string matrixName, int block_i,int block_j,vector<vector<int>> rows, int rowStart,int columnStart,int rowEnd,int columnEnd);
};